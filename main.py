...
btn_dialog = ttk.Button(frame_folder, text="Вибрати директорії секцій", command=choose_dir)
btn_dialog.pack(padx=0, fill=X)

# списки секцій з та без рі
section_list_s = ttk.Label(frame_section_list,
                           text='Для наступних секцій буде спаковано PI та креслення (при наявності):')
section_list_s.pack(padx=5, fill=X)

section_list_success = Text(frame_section_list, height=3, width=35, padx=2, pady=0, wrap=WORD, relief=SOLID,
                            borderwidth=1)
...
section_list_fail = Text(frame_section_list, height=3, width=35, padx=2, pady=0, wrap=WORD, relief=SOLID, borderwidth=1)
...
result_packed = Text(frame_result, height=2, width=35, padx=2, pady=0, relief=SOLID, borderwidth=1)
